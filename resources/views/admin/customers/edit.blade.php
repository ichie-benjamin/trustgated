            ...
            <div class="form-layout form-layout-4">
                ...
                <form action="{{ route('admin.customers.update', $customer) }}" method="POST">
                    ...
                    <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                        <input id="name" type="text" placeholder="Fullname" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ $customer->name }}" required autocomplete="name" autofocus>
                    ...
                    <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                        <input id="email" type="email" placeholder="Email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ $customer->email }}" readonly required autocomplete="email" autofocus>
                    ...
                    <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                        <input id="phone" type="text" placeholder="Phone number" class="form-control @error('phone') is-invalid @enderror" name="phone" value="{{ $customer->profile->phone }}" required autocomplete="phone" autofocus>
                    ...
                    <div class="row mg-t-20">
                        ...
                        <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                            <input id="name" type="text" placeholder="Username" class="form-control @error('username') is-invalid @enderror" name="username" value="{{ $customer->profile->username }}" required readonly autocomplete="username" autofocus>
                    ...
                    <div class="row mg-t-20">
                        <label class="col-sm-4 form-control-label"> <span>Active</span>    </label>                      <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                            <input type="checkbox" name="active" @if ($customer->profile->active) checked @endif>
                        </div>
                    </div>
                        ...
                        <select name="delivery_category_id" class="form-control select2" data-placeholder="Choose Delivery Package">
                            <option label="Choose Delivery Package"></option>
                                @foreach ($deliveryPackages ?? [] as $package)
                                    <option value="{{ $package->id ?? '' }}" @if($package->id == ($customer->delivery_package->id ?? 0)) selected @endif>{{ $package->name }}</option>    
                                @endforeach
